...
function AccessControl() {
  const [roles, _setRoles] = useState<RichViewType[]>();
  const [jobs, setJobs] = useState<RichViewType[]>();
  const [organizationUnits, _setOrganizationUnits] = useState<RichViewType[]>();
  const [loading, setLoading] = useState(false);
  const [list, setList] = useState<AccessControlListResponseType[]>();
  const [addNewAccess, setAddNewAccess] = useState(false);
  const [_filter, setFilter] = useState<AccessControlFilterType>();
  ...
  const columns = useMemo(() => {
    return [
      ...
      },
      {
        ...
        Cell: ({ row }: { row: { original: { status: string , id: number, showPassword: boolean} } }) => {
          return (
            <FormControlLabel
              control={<Checkbox checked={row.original.status=='active'} />}
              label={
                row.original.status
          ...
          {list && (
            <MyCustomTable
              ...
              rowActions={({
                ...
                <IconButton
                  color="secondary"
                  onClick={() => setSelectedUserGrants(String(row.original.id))}
                >
                  <Edit />
                </IconButton>
              )}
